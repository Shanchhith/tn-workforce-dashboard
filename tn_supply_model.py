...
# ============================================================================
# 6b. REVISED PROJECTED NEED: demand side of the health labour market model
# ============================================================================
...
RN_E_GDP   = 0.231 + 0.531 - 0.518   # long-run income elasticity, 0.244
RN_E_OOP   = -0.099                  # out-of-pocket spending per capita
RN_E_POP65 = 0.516                   # absolute population aged 65 and over
RN_GDP_G_2026 = 0.065   # real GSDP per capita growth, 2026. ASSUMED, see Assumptions
RN_GDP_G_2050 = 0.040   # eases linearly to this by 2050. ASSUMED
RN_OOP_ADJ    = 0.0     # OOP per capita growth minus GDP per capita growth. ASSUMED
RN_POP65_G    = 0.035   # growth of the 65+ population per year. ASSUMED
RN_ANCHOR     = 2025

def build_revised_need(active):
    """Revised projected need: the doctors the state's own economy will support,
    on the demand side of the World Bank health labour market model, anchored on
    the modelled 2025 density. NaN before the anchor year."""
    ...
    for i in range(i0 + 1, len(YEARS)):
        y = YEARS[i]
        g = RN_GDP_G_2026 + (RN_GDP_G_2050 - RN_GDP_G_2026) * (y - 2026) / (2050 - 2026)
        dens *= ((1 + g) ** RN_E_GDP * (1 + g + RN_OOP_ADJ) ** RN_E_OOP
                 * (1 + RN_POP65_G) ** RN_E_POP65)
        out[i] = dens * POP[i] / 10000.0
    return out

REVISED_NEED = build_revised_need(np.array([r["active"] for r in RESULTS["Status quo"]], float))
...
with open("tn_projection_2050.csv", "w", newline="") as f:
    w = csv.writer(f)
    hdr = ["year", "population", "who_need_doctors", "revised_need_doctors",
    ...
    for i, y in enumerate(YEARS):
        ...
        row = [y, f"{POP[i]:.0f}", f"{NEED_DOCTORS[i]:.0f}",
               "" if np.isnan(REVISED_NEED[i]) else f"{REVISED_NEED[i]:.0f}",
...
for s in SCENARIOS:
    ...
    line("Revised projected need", REVISED_NEED)
    line("Surplus vs revised need", a - REVISED_NEED)
